...
#import "MecidiyekoyViewController.h"

@interface MecidiyekoyViewController ()

@end

@implementation MecidiyekoyViewController
...
- (void) updateKustepe{
    ...
    if ([today isEqualToString:Saturday]) {
        [mecidiyekoyKustepe setText:@"No Bus"];
    }
    else if([today isEqualToString:Sunday]){
        [mecidiyekoyKustepe setText:@"No Bus"];
    }
    
    else if (ringshuttle>=800 && ringshuttle<1730) {
        [mecidiyekoyKustepe setText:@"Ring"];
    }
    
    else{


        [self updateClock: [NSArray arrayWithObjects:[NSNumber numberWithInt:8],[NSNumber numberWithInt:18], [NSNumber numberWithInt:19],[NSNumber numberWithInt:19],[NSNumber numberWithInt:20],nil]: [NSArray arrayWithObjects:[NSNumber numberWithInt:0],[NSNumber numberWithInt:10],[NSNumber numberWithInt:0],[NSNumber numberWithInt:50],[NSNumber numberWithInt:40], nil]:mecidiyekoyKustepe];
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    [self updateKustepe];
	timerb= [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(updateKustepe) userInfo:nil repeats:YES];}
